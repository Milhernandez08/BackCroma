...
const crear = (request, response) => {
    ...
    if (id_lote && loc_gps && profundidad && fecha && asnm && clima && uso_suelo && tipo_suelo && num_img) {
        connection_1.pool.query('INSERT INTO muestra(id_lote, loc_gps, profundidad, fecha, asnm, clima, uso_suelo, tipo_suelo, num_img) VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9)', [id_lote, loc_gps, profundidad, fecha, asnm, clima, uso_suelo, tipo_suelo, num_img], (error, results) => {
            if (error) {
                throw error;
            }
...
const todos = (request, response) => {
    connection_1.pool.query('SELECT * FROM muestra', (error, results) => {
        if (error) {
            throw error;
        }
...
const porId = (request, response) => {
    ...
    connection_1.pool.query('SELECT * FROM muestra WHERE id=$1', [id], (error, results) => {
        if (error) {
            throw error;
        }
        response.status(200).json(results.rows);
...
const editar = (request, response) => {
    ...
    if (loc_gps && profundidad && fecha && asnm && clima && uso_suelo && tipo_suelo && num_img) {
        connection_1.pool.query('UPDATE muestra SET pais=$1, estado=$2, municipio=$3, nombre_lugar=$4 WHERE id=$5', [loc_gps, profundidad, fecha, asnm, clima, uso_suelo, tipo_suelo, num_img, id], (error, results) => {
            if (error) {
                throw error;
            }
